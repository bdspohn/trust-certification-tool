...
export default function TermsOfService() {
  ...
  useEffect(() => {
    async function loadTerms() {
      try {
        const document = await fetchBontermsTemplate('terms-of-service');
        setTerms(document);
      } catch (err) {
  ...
  if (error) {
    return (
      <div className="max-w-3xl mx-auto p-8 bg-white rounded-lg shadow mt-8">
        <h1 className="text-3xl font-bold mb-4 text-red-600">Error Loading Terms of Service</h1>
    ...
    <div className="max-w-3xl mx-auto p-8 bg-white rounded-lg shadow mt-8">
      ...
      <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        ...
        <div className="text-xs text-blue-600 space-y-1">
          <p>Template ID: {terms.meta.templateId}</p>
          <p>Last Updated: {terms.meta.lastUpdated}</p>
          <p>Customized for: {terms.meta.customizedFor}</p>
      ...
      <div className="mt-8 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
        <p className="text-sm text-yellow-800">
          ...
          If you have questions about these terms, contact us at legal@trusto.com.
        </p>
      </div>
    </div>
  );
